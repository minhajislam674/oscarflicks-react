...
export const MovieView = ({movieData}) => {
    ...
    const addToFavoriteSuccess = () => toast.info("Added to favorites!", {
      position: "top-right",
      autoClose: 2000,
    ...
    const handleFavorite = async () => {
        ...
        const response = await fetch("https://myflix-movies.onrender.com/users/"+user.Username, {
                method: "GET",
                headers: {
              ...
              console.log(favoriteMovies);
                if (!favoriteMovies.includes(movie._id)) {
                    setFavorite(true);
                    await fetch("https://myflix-movies.onrender.com/users/"+user.Username+"/movies/"+movie._id, {
                        method: "POST",
                        headers: {
                          Authorization: `Bearer ${token}`,
                          "Content-Type": "application/json"
    ...
    return (
            <Container>
                    ...
                    <Row>
                        <Col xs={12} lg={6} md={9}>
                            <CardImg className="card-image-movie-view" src={movie.ImagePath} alt="movie-poster"/>
                        </Col>
                        <Col xs={12} lg={6} md={9}> 
                            <Card.Body className="card-body">
                                <Card.Title className="card-title-movie-view">{movie.Title} ({movie.ReleaseYear}) 
                                <img src={favorite ? favorited : notFavorited} alt="favorite-icon" className="icon-favorite"  title="Add to favorite" onClick={handleFavorite}/>
                                </Card.Title>
